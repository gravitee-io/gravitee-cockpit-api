...
public class UserReplyAdapter
  implements
    ReplyAdapter<UserReply, io.gravitee.cockpit.api.command.v1.user.UserReply> {
  ...
  public Single<io.gravitee.cockpit.api.command.v1.user.UserReply> adapt(
    ...
    return Single.fromCallable(() -> {
      if (reply.getCommandStatus() == CommandStatus.ERROR) {
        return new io.gravitee.cockpit.api.command.v1.user.UserReply(
          reply.getCommandId(),
          reply.getMessage()
        );
      } else {
